...
//heap insert as discussed has a time complexity of O(log(n)) for every individual element and O(nlog(n)) for n elements. This is due to the fact that each of the n elements has to traverse down levels of the tree that fluctuate with O(log(n))(base 2).
...
int main()
{
    ...
    for (n = 10000; n <= 100000; n = n+10000){
      ...
      //log(n)/log(2) gives us log(n) in base 2
      printf("n = %6d, %lf %lf\n", n, xc/(n*100.0), yc/(n*100.0));
      printf("            %lf %lf\n", xc1/(n*((log(n)/log(2)))*100.0), yc1/(n*((log(n)/log(2)))*100.0));
      printf("----\n");
     }
     //The math in this case (of what i am able to make out), gives us the propotional amount of comparisons and swaps with regard to n(and its appropriate functions) for our different input sizes. 
}    //we see that heap insert keeps around 2.27 comparisons and 1.27 swaps on average while heapify does much better at around 1 comparison and 0.74 swaps on average. Deletion in both cases stays about the same.

//heapsort takes place when we heapinsert then heap delete or when we heapify then delete into an array thus we see The wole process averaging around 4 comparisons and 2 swaps for our heapinsert case and less than 3 comparisons and around 1.5 swaps for our heapfy case.
